...
@Client.on_message(filters.command(["msettings", "mconfig", f"mconfig@{Config.BOT_USERNAME}", f"msettings@{Config.BOT_USERNAME}"]) & admin_filter & chat_filter)
async def settings(client, m: Message):
    await m.reply(f"Configure Your VCPlayer Settings.", reply_markup=await settings_panel(), disable_web_page_preview=True)
    await delete_messages([m])
